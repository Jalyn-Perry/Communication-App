...
A simple **chat application** in Python using TCP sockets and threading.  
...
2. Enter the IP address and port to bind the server to (use `127.0.0.1` for local testing).
3. Wait for a client to connect.

### Client Setup

1. Run the script and choose `2` to start the client.
2. Enter the server’s IP address and port to connect.
...
- Open **two terminal windows**.
  - One for the server
  - One for the client
  - The terminals are needed to start the script! The GUIs will be used to communicate 
